...
func mergeSort(refSlice []int) []int { //recursive function - calling itself
	if len(refSlice) < 2 {
		return refSlice
	} // if there is only 1 item in the slice, return the slice value, no need for further merging
...
func merge(topHalf []int, bottomHalf []int) []int {
	...
	for topHalfIndex < len(topHalf) && bottomHalfIndex < len(bottomHalf) { //keeps looping through if index cycle value is still less than the reference slice
		...
		} else { //if bottom half value is smaller than top half value
